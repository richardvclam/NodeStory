import crypto from 'crypto';

import { AESEncryption } from '../security/AESEncryption';
import * as ShandaEncryption from '../security/ShandaEncryption';

export class PacketCrypto {
  constructor(
    private version: number,
    private aes: AESEncryption,
    private iv: Uint8Array = crypto.randomBytes(4),
  ) {}

  public generateHeader(length: number): Buffer {
    let a = this.iv[2] | (this.iv[3] << 8);

    a ^= -(this.version + 1);
    const b = a ^ length;

    const header = Buffer.from([
      a & 0xff,
      (a >>> 8) & 0xff,
      b & 0xff,
      (b >>> 8) & 0xff,
    ]);
    return header;
  }

  public encryptData(data: Buffer): Buffer {
    // const encryptedData = cryptoConfig.enableShandaEncryption
    //   ? ShandaEncryption.encrypt(data)
    //   : data;

    const encryptedData = ShandaEncryption.encrypt(data);
    const result = this.aes.transform(encryptedData, this.iv);
    this.iv = this.morphIV();

    return result;
  }

  public decryptData(data: Buffer): Buffer {
    const transformedData = this.aes.transform(data, this.iv);
    this.iv = this.morphIV();

    // return cryptoConfig.enableShandaEncryption
    //   ? ShandaEncryption.decrypt(transformedData)
    //   : transformedData;
    return ShandaEncryption.decrypt(transformedData);
  }

  public morphIV(): Uint8Array {
    const newSequence = new Uint8Array([0xf2, 0x53, 0x50, 0xc6]);

    for (let i = 0; i < 4; i++) {
      const input = this.iv[i];
      const tableInput = ivShiftingKey[input];
      newSequence[0] += ivShiftingKey[newSequence[1]] - input;
      newSequence[1] -= newSequence[2] ^ tableInput;
      newSequence[2] ^= ivShiftingKey[newSequence[3]] + input;
      newSequence[3] -= newSequence[0] - tableInput;

      let val =
        (newSequence[0] |
          ((newSequence[1] & 0xff) << 8) |
          ((newSequence[2] & 0xff) << 16) |
          ((newSequence[3] & 0xff) << 24)) >>>
        0;
      let val2 = val >>> 0x1d;
      val = (val << 0x03) >>> 0;
      val2 |= val;
      newSequence[0] = val2 & 0xff;
      newSequence[1] = (val2 >> 8) & 0xff;
      newSequence[2] = (val2 >> 16) & 0xff;
      newSequence[3] = (val2 >> 24) & 0xff;
    }

    return newSequence;
  }

  public getIV(): Uint8Array {
    return this.iv;
  }
}

const ivShiftingKey = new Uint8Array([
  ,
]);
