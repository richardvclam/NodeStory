import { PacketCrypto } from './PacketCrypto';
...
describe("PacketCrypto", () => {
  it("packet should encrypt properly", () => {
    const block = Buffer.from(decryptedLoginResponse);
    const crypto = new PacketCrypto(83, iv);
    const data = crypto.encryptData(block);

    expect(data.toJSON().data).toEqual(encryptedLoginResponse);
  });

  it("packet should decrypt properly", () => {
    const block = Buffer.from(encryptedLoginResponse);
    const crypto = new PacketCrypto(83, iv);
    const data = crypto.decryptData(block);
